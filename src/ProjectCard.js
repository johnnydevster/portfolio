    ...
    <div className="my-8 bg-blue-900 bg-opacity-40 p-4 mx-auto rounded">
      ...
      <div className={`relative flex ${isEven && "justify-end"}`}>
        ...
        <div
          ...
          } m-auto text-sm px-5 pb-5 pt-3`}
        >
          {props.paragraph.map((paragraph) => {
            return <p className="pt-2">{paragraph}</p>;
          })}
        </div>
      </div>
      <div
      ...
      >
        {props.tech.map((tech) => {
          return (
            <div className="mt-2 transition-all duration-400 ease-in-out h-9 bg-indigo-600 mx-1 px-3 rounded pt-2 hover:px-5 hover:bg-indigo-500 hover:text-white">
              {tech}
            </div>
          );
        })}
      </div>
      <div className="mt-5 text-sm font-semibold flex bg-blue-900 rounded bg-opacity-40">
        <a
          href={props.repolink}
          target="_blank"
          className="flex text-yellow-300 py-1 px-2 rounded-2xl hover:text-yellow-200"
        >
          <p>code</p>
